...
class WC_Gateway_Skye_Online extends WC_Payment_Gateway {
  ...
  /** 
  * Skye price info widgets for product page
  */
  public function skye_price_widget(){
    global $product;
    if(isset($this->settings['price_widget']) && $this->price_widget =='yes'){          
      $minimum = $this->get_min_price();
      $maximum = $this->get_max_price();
      $price = wc_get_price_to_display($product);
      $this->log->add( $this->id, 'Add price widget:'.$price );
      if(($minimum == 0 || $price >= $minimum) && ($maximum == 0 || $price <= $maximum)) {
        return '<style>#skye-tag p img { display: inline; } #skye-tag p {margin: 1em 0px 1em 0px;}</style><div id="skye-price-anchor"></div><script id="skye-widget" data-min="'.$this->skye_minimum.'" data-max="'.$this->skye_maximum.'" src="https://d1y94doel0eh42.cloudfront.net/content/scripts/skye-widget.js?id='.$this->settings['merchant_id'].'&productPrice='.$price.'&element=%23skye-price-anchor&term='.$this->settings['skye_promo_term'].'&mode='.$this->settings['price_widget_mode'].'"></script>';
      }else{
        return null;
      }
    }else{
  ...
  /**
  * Skye price info widgets for checkout
  */
  public function skye_checkout_price_widget(){
    if(isset($this->settings['price_widget']) && $this->price_widget =='yes'){   
      $minimum = $this->get_min_price();
      $maximum = $this->get_max_price();
      $price = WC()->cart->total;
      if(($minimum == 0 || $price >= $minimum) && ($maximum == 0 || $price <= $maximum)) {
        return '<style>#skye-tag p img { display: inline; } #skye-tag p {margin: 1em 0px 1em 0px;}</style><div id="skye-price-anchor"></div><script id="skye-widget" data-min="'.$this->skye_minimum.'" data-max="'.$this->skye_maximum.'" src="https://d1y94doel0eh42.cloudfront.net/content/scripts/skye-widget.js?id='.$this->settings['merchant_id'].'&productPrice='.$price.'&element=tr.order-total%20span.woocommerce-Price-amount.amount&term='.$this->settings['skye_promo_term'].'&mode='.$this->settings['price_widget_mode'].'"></script>';
      }else{
        return null;
      }
    }else{
  ...
  */
  ...
  private function extra_details( $order_id = '' ) {
    echo '<h2>' . __( 'Thank you for your order!', 'woocommerce-skye-online' ) . '</h2>' . PHP_EOL;
  ...
  private function format_address($address_parts, $order) {
    $street_types = WC_Skye_Online_Street_Type();
    ...
    $this->log->add( $this->id, 'Address street type->' . $address_type_str);
